...
# rrcf_result_file = Path("./../results/rrcf_results_station_dif")
...
# num_trees_opt = [150]
# run_window = 25000
# tree_size_opt = [15000, 20000, 25000]
# shingle_size_opt = [1]

# test_sample_file = Path("../stationdif_test_sections.csv")
...
def get_f_value(df_c: DataFrame, rel_cod_threshold, a_threshold: AThreshold):
    ...
    nmbr_true_pos = df_c[(abs_codisp > rel_cod_threshold) & (abs(df_c[a_threshold.name]) > a_threshold.value)].shape[0]
    recall = nmbr_true_pos / df_c[abs(df_c[a_threshold.name]) > a_threshold.value].shape[0]
...
def calc_mcc(df_points, avg_codisp_a, cod_threshold, a_threshold, shingle_size):
    ...
    tp = df_p[(avg_codisp >= cod_threshold) & (abs(df_p[a_threshold.name]) >= a_threshold.value)].shape[0]
    tn = df_p[(avg_codisp < cod_threshold) & (abs(df_p[a_threshold.name]) < a_threshold.value)].shape[0]
    fp = df_p[(avg_codisp >= cod_threshold) & (abs(df_p[a_threshold.name]) < a_threshold.value)].shape[0]
    fn = df_p[(avg_codisp < cod_threshold) & (abs(df_p[a_threshold.name]) >= a_threshold.value)].shape[0]
...
def save_result(station, start_date, end_date, tree_size, nmbr_trees, shingle_size, cod_threshold, mcc, acc, tp, tn, fp,
                fn, calc_nmbr, phase, a_type, t_constructed, t_needed, rrcf_result_file):
    with lock:
        if rrcf_result_file.exists():
            ...
            df = pd.DataFrame(columns=['station', 'start_date', 'end_date', 'tree_size', 'nmbr_trees', 'shingle_size',
                                       'cod_threshold', 'mcc', 'acc', 'tp', 'tn', 'fp', 'fn', 'calc_nmbr', 'phase',
                                       'a_name', 'c_time', 'r_time'])
        df = df.append({'station': station, 'start_date': start_date, 'tree_size': tree_size,
                        ...
                        'mcc': mcc, 'acc': acc, 'tp': tp, 'tn': tn, 'fp': fp, 'fn': fn, 'calc_nmbr': calc_nmbr,
                        'phase': phase, 'a_name': a_type.name,
...
def run_hp_test(df_constr, df_p, station, start_date, end_date, tree_size, nmbr_trees, shingle_size, a_type, calc_nmbr,
    ...
    save_result(station, start_date, end_date, tree_size, nmbr_trees, shingle_size, cod_threshold, mcc, acc, tp, tn, fp,
                fn, calc_nmbr, phase, a_type, t_constructed, t_needed, result_file)
...
def test_h_parameters(test_params):
    for tp in test_params:
        sample_df = pd.read_csv(tp.test_sample_file)
        for index, row in sample_df.iterrows():
            ...
            phase = row['phase']
            for tn in tp.num_trees_opt:
                for ts in tp.tree_size_opt:
                    for ss in tp.shingle_size_opt:
                        for pn in range(tp.reps):
                            df_p, df_const = get_test_sample(station, start, end, a_type, ts, ss, phase, tp.run_window)
                            calc_nmbr = np.random.randint(low=0, high=99999999)
                            p = Process(target=run_hp_test, args=(
                                df_const, df_p, station, start, end, ts, tn, ss, a_type, calc_nmbr, phase,
                                tp.result_file))
                            start_process(p)
...
def main():
    ...
    #        print(time.time())
    #        time.sleep(10)
    # tp1 = TestParam([30000], 25000, [100,150,200,250,300], [1], Path("../phasedif_test_sections.csv"),
    #                 Path("./../results/rrcf_results_phase_dif2_nt"), 3)
    tp4 = TestParam([15000, 20000, 25000, 30000, 35000, 40000, 45000, 50000, 55000], 25000, [150], [1], Path("../seasdif_test_sections.csv"),
                    Path("./../results/rrcf_results_seas_dif"), 3)
    tp5 = TestParam([15000, 20000, 25000, 30000, 35000, 40000, 45000, 50000, 55000], 25000, [150], [1], Path("../stationdif_test_sections.csv"),
                    Path("./../results/rrcf_results_station_dif"), 3)
    tp6 = TestParam([25000, 30000, 35000, 40000, 45000], 25000, [150], [1], Path("../trafo_test_sections.csv"),
                    Path("./../results/rrcf_results_trafo_dif"), 3)
    test_params = [tp4, tp5]
